...
Thesis: *Semantic Characterization of Robotic Environments*, an open-vocabulary scene graph generation system for robotic manipulation using CLIP, SAM, Grounding DINO, and RAM (MERLIN Lab, Politecnico di Milano). Currently extending it with **ROS2** integration.

Open to roles in **Automation &amp; Control / Robotics Engineering** and **AI / ML Engineering**, where my control background adds an edge.
...
|---|---|---|
| [Lightweight-SGG](https://github.com/Romi-s/Lightweight-SGG) | CPU-only service detecting objects and their spatial relations. Shows that bounding-box geometry plus a class prior beats CLIP image-text similarity for spatial predicates (0.79 zero-shot, 0.91 with a tiny trained head). FastAPI + Docker + CI, 21 tests, [live Gradio demo](https://huggingface.co/spaces/Romi-s/light-SGG-demo) | Python · PyTorch · YOLO-World · FastAPI · Docker |
...
| [Time-Series-Forecasting](https://github.com/Romi-s/Time-Series-Forecasting) | Benchmarked 6+ architectures (LSTM, Conv. LSTM, Transformer) on 48K time series with autoregressive decoding (test MSE 0.01) | Python · TensorFlow · Keras · NumPy |

**Also:** a *Scene Graph Generation* MSc thesis (4 chained foundation models, 6× over an end-to-end baseline; code under MERLIN Lab), an *Agentic Job-Application Automation System* (headless LLM agents over a Markdown knowledge base, scheduled workflows, git review gate), and control/robotics coursework including a *2-DOF pantograph CNC* on real Quanser hardware and *networked control of a vehicle platoon*. More on the [portfolio](https://romi-s.github.io).
